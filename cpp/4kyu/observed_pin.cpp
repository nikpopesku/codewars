...
void backtrack(const string& combination, string& observed, const int index, vector<string>& values)
{
    ...
    for (const char c : mp.at(observed[index]))
    {
        backtrack(combination + c, observed, index + 1, values);
    }
}
